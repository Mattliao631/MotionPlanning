...
public class PlayerController : MonoBehaviour
{
    ...
    void Down_Smash(){
        if (Input.GetKeyDown("s") && (animator.GetCurrentAnimatorStateInfo(0).IsName("HeroKnight_Jump") || animator.GetCurrentAnimatorStateInfo(0).IsName("HeroKnight_Fall")))
        {
            float since_last_s = Time.time - last_s_timer;
            if(since_last_s <= double_s_range){
                // do the skill
                Debug.Log("down_smashing");
                down_smashing = true;
                max_drop_velocity = max_drop_velocity * 4;
                Vector2 playerVal = new Vector2(rb.velocity.x, max_drop_velocity);
                rb.velocity = playerVal;
                max_drop_velocity = max_drop_velocity/4;
            }
